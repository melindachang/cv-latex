...
GPA: $education.gpa$$if(education.subtitle)$, $education.subtitle$$endif$$endif$\\[.2cm]
$endfor$
$endif$
...
$if(experience.advisor)$Advisor: $experience.advisor$. $endif$$if(experience.description)$$experience.description$\\$endif$
$experience.city$\\[.2cm]
$endfor$
...
\noindent
$for(publication)$
\note{$publication.year$}$publication.authors$, \href{publication.doi}{``$publication.title$.''} $publication.venue$.\\[.2cm]
$endfor$
...
$for(award)$
\note{$award.year$}$award.name$$if(award.organization)$, \textit{$award.organization$}$endif$$if(award.amount)$, $award.amount$$endif$.\\[.2cm]
$endfor$
$endif$
...
$for(projects)$
\note{$projects.year$}\href{$projects.url$}{\emph{$projects.name$}}. $projects.description$\\[.2cm]
$endfor$
$endif$
